...
<div class="min-h-screen bg-gray-950 py-12">
    <div class="container mx-auto px-4 max-w-5xl">
        ...
        <!-- Profile Header -->
        <div class="bg-gray-900 rounded-3xl shadow-xl overflow-hidden mb-8 border border-gray-800">
            <div class="bg-gray-950 h-32 relative">
                <div class="absolute inset-0 bg-gradient-to-r from-yellow-600/20 to-gray-950/50"></div>
            </div>
            <div class="px-8 pb-8 relative">
                <div
                    class="-mt-16 flex flex-col md:flex-row items-center md:items-start gap-6 text-center md:text-left">
                    <div class="relative group">
                        <div
                            class="w-32 h-32 rounded-full border-4 border-gray-900 bg-gray-800 flex items-center justify-center text-gray-400 text-4xl shadow-lg relative shrink-0 overflow-hidden">
                            <?php
                            $profile_img = 'profileimg/' . $user_id . '.jpg';
                            if (file_exists($profile_img)):
                                ?>
                                <img src="<?php echo $profile_img . '?' . time(); ?>" alt="Profile"
                                    class="w-full h-full object-cover">
                            <?php else: ?>
                                <span
                                    class="font-bold text-5xl text-gray-500"><?php echo strtoupper(substr($user['username'], 0, 1)); ?></span>
                            <?php endif; ?>
                            <!-- Edit Overlay -->
                        ...
                        <?php if ($user['is_verified']): ?>
                            <div class="absolute bottom-1 right-1 w-8 h-8 bg-blue-500 rounded-full flex items-center justify-center text-white text-sm border-2 border-gray-900 z-10"
                        ...
                        <button onclick="document.getElementById('profile_upload').click()"
                            class="absolute top-0 left-0 w-8 h-8 bg-gray-800 text-white rounded-full flex items-center justify-center border-2 border-gray-900 shadow-md hover:bg-gray-700 transition-colors z-20"
                            title="Change Profile Picture">
                            <i class="fas fa-camera text-xs"></i>
                        </button>

                        <!-- Hidden Upload Form -->
                        <form id="profile_form" action="upload_profile.php" method="POST" enctype="multipart/form-data"
                            class="hidden">
                            <input type="file" name="profile_image" id="profile_upload"
                                accept="image/jpeg,image/png,image/webp"
                                onchange="document.getElementById('profile_form').submit()">
                        </form>

                        <!-- Remove Button (Top Right) - Only show if image exists -->
                        <?php if (file_exists('profileimg/' . $user_id . '.jpg')): ?>
                            <button onclick="document.getElementById('removeProfileModal').classList.remove('hidden')"
                                class="absolute top-0 right-0 w-8 h-8 bg-red-600/80 text-white rounded-full flex items-center justify-center border-2 border-gray-900 shadow-md hover:bg-red-600 transition-colors z-20"
                    ...
                    <div class="flex-grow pb-2 md:mt-20">
                        <div class="flex flex-col md:items-start items-center">
                            <h1 class="text-4xl font-bold font-serif text-white mb-3 flex items-center gap-3">
                                <?php echo htmlspecialchars($user['username']); ?>
                                <?php if (isset($user['role']) && $user['role'] === 'admin'): ?>
                                    <span
                                        class="bg-red-900/30 text-red-500 text-xs px-3 py-1 rounded-full font-sans font-bold uppercase tracking-wider border border-red-900/50">Admin</span>
                                <?php endif; ?>
                            </h1>
                            <div class="flex flex-wrap justify-center md:justify-start gap-3 text-gray-400">
                                <div
                                    class="flex items-center gap-2 bg-gray-800 px-4 py-2 rounded-xl border border-gray-700 shadow-sm transition-colors hover:bg-gray-700 hover:shadow-md">
                                ...
                                <div
                                    ...
                                    <span class="font-medium text-sm">Joined
                                        <?php echo date('F Y', strtotime($user['created_at'])); ?></span>
                                </div>
                            </div>
                        </div>
                    </div>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
            ...
            <div class="lg:col-span-1 space-y-6">
                ...
                <div class="bg-gray-900 rounded-2xl shadow-lg p-6 border border-gray-800">
                    <h3 class="font-bold text-white mb-4 font-serif">Your Activity</h3>
                    <div class="space-y-4">
                        <div class="flex items-center justify-between p-3 bg-gray-800 rounded-xl">
                            <div class="flex items-center gap-3">
                                <div
                                    class="w-10 h-10 rounded-full bg-blue-900/30 text-blue-400 flex items-center justify-center">
                                    <i class="fas fa-bed"></i>
                                </div>
                                <span class="text-sm font-medium text-gray-400">Room Stays</span>
                            </div>
                            <span class="font-bold text-white"><?php echo $stats['rooms']; ?></span>
                        </div>
                        <div class="flex items-center justify-between p-3 bg-gray-800 rounded-xl">
                            <div class="flex items-center gap-3">
                                <div
                                    class="w-10 h-10 rounded-full bg-yellow-900/30 text-yellow-500 flex items-center justify-center">
                                    <i class="fas fa-utensils"></i>
                                </div>
                                <span class="text-sm font-medium text-gray-400">Dining</span>
                            </div>
                            <span class="font-bold text-white"><?php echo $stats['dining']; ?></span>
                        </div>
                        <div class="flex items-center justify-between p-3 bg-gray-800 rounded-xl">
                            <div class="flex items-center gap-3">
                                <div
                                    class="w-10 h-10 rounded-full bg-emerald-900/30 text-emerald-400 flex items-center justify-center">
                                    <i class="fas fa-spa"></i>
                                </div>
                                <span class="text-sm font-medium text-gray-400">Spa Visits</span>
                            </div>
                            <span class="font-bold text-white"><?php echo $stats['spa']; ?></span>
                        </div>
                    </div>
                </div>
                    ...
                    <div class="space-y-2">
                        <a href="../booking/view_bookings.php"
                            ...
                            <span><i class="fas fa-calendar-check mr-3 text-gray-500 group-hover:text-yellow-600"></i>My
                                Bookings</span>
                            <i class="fas fa-chevron-right text-xs text-gray-600 group-hover:text-gray-400"></i>
                        </a>
                        <a href="settings.php"
                            class="block w-full text-left px-4 py-3 rounded-xl hover:bg-gray-800 text-gray-300 font-medium transition-colors flex justify-between items-center group">
                            <span><i
                                    class="fas fa-cog mr-3 text-gray-500 group-hover:text-yellow-600"></i>Settings</span>
                            <i class="fas fa-chevron-right text-xs text-gray-600 group-hover:text-gray-400"></i>
                        </a>
                    </div>
                </div>
            ...
            <div class="lg:col-span-2">
                ...
                <div class="bg-gray-900 rounded-2xl shadow-lg p-8 border border-gray-800 mb-8">
                        ...
                        <div class="space-y-4">
                            <?php foreach ($recent_activities as $activity): ?>
                                ...
                                if ($activity['type'] == 'room') {
                                    ...
                                    if (isset($activity['reservation_time']))
                                        $subtext .= ' at ' . date('g:i A', strtotime($activity['reservation_time']));
                                } elseif ($activity['type'] == 'spa') {
                                    $icon = 'fa-spa';
                                    $bg_color = 'bg-emerald-900/30';
                                    $text_color = 'text-emerald-400';
                                    $subtext = 'Appointment: ' . date('M d, Y', strtotime($activity['date']));
                                    if (isset($activity['spa_time']))
                                        $subtext .= ' at ' . date('g:i A', strtotime($activity['spa_time']));
                                }
                                ...
                                ?>
                                <div
                                    class="flex items-center p-4 bg-gray-800 border border-gray-700 rounded-xl hover:shadow-md transition-shadow group">
                                    <div
                                        class="w-12 h-12 rounded-full <?php echo $bg_color . ' ' . $text_color; ?> flex items-center justify-center mr-4 shrink-0 group-hover:scale-110 transition-transform">
                                    ...
                                    <div class="ml-4 shrink-0">
                                        <span
                                            class="px-3 py-1 rounded-full text-[10px] md:text-xs font-bold uppercase border <?php echo $status_class; ?>">
                    ...
                    <?php else: ?>
                        <div class="text-center py-12 bg-gray-800 rounded-2xl border border-dashed border-gray-700">
...
<div id="removeProfileModal"
    class="fixed inset-0 bg-black/80 z-50 hidden flex items-center justify-center backdrop-blur-sm p-4">
    <div
        class="bg-gray-900 rounded-2xl shadow-2xl max-w-sm w-full p-6 border border-gray-800 transform transition-all scale-100">
        <div class="text-center mb-6">
            <div
                class="w-16 h-16 bg-red-900/30 rounded-full flex items-center justify-center mx-auto mb-4 text-red-500">
                <i class="fas fa-trash-alt text-2xl"></i>
            </div>
            <h3 class="text-xl font-bold text-white mb-2">Remove Profile Picture?</h3>
            <p class="text-gray-400 text-sm">Are you sure you want to remove your profile picture? This action cannot be
                undone.</p>
        </div>
        <div class="flex gap-3">
            <button onclick="document.getElementById('removeProfileModal').classList.add('hidden')"
                class="flex-1 px-4 py-2.5 bg-gray-800 text-gray-300 font-bold rounded-xl hover:bg-gray-700 transition-colors">
                Cancel
            </button>
            <form action="remove_profile_pic.php" method="POST" class="flex-1">
                <button type="submit"
                    class="w-full px-4 py-2.5 bg-red-600 text-white font-bold rounded-xl hover:bg-red-700 transition-colors shadow-lg shadow-red-500/30">
                    Yes, Remove
                </button>
            </form>
        </div>

...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $alter = "ALTER TABLE bookings ADD COLUMN IF NOT EXISTS notification_seen TINYINT(1) DEFAULT 0";
    @mysqli_query($conn, $alter);

    // Ensure status_updated_at column exists (best-effort)
    $alter2 = "ALTER TABLE bookings ADD COLUMN IF NOT EXISTS status_updated_at DATETIME DEFAULT NULL";
    @mysqli_query($conn, $alter2);

    // Update booking status
    $sql = "UPDATE bookings SET status = ?, status_updated_at = NOW(), notification_seen = 0 WHERE id = ?";
    $stmt = $conn->prepare($sql);

    // If prepare failed (e.g. missing column despite ALTER attempts), try fallback immediately
    if (!$stmt) {
        $fallback = "UPDATE bookings SET status = '$status' WHERE id = $booking_id";
        if (mysqli_query($conn, $fallback)) {
            http_response_code(200);
            exit('Success (Fallback directly)');
        }
    ...
    if ($stmt->execute()) {
        ...
        // Fallback if columns are still missing or other SQL error
        // Try simplest update
        $fallback = "UPDATE bookings SET status = '$status' WHERE id = $booking_id";
        if (mysqli_query($conn, $fallback)) {
            http_response_code(200);
            exit('Success (Fallback)');
        }

                ...
                <div class="room_card group bg-gray-800/50 backdrop-blur-sm rounded-3xl overflow-hidden shadow-lg hover:shadow-2xl transition-all duration-500 flex flex-col h-full transform hover:-translate-y-2 room-clickable"
                    ...
                    <div class="p-8 flex flex-col flex-grow">
                        ...
                        <div class="mt-auto space-y-3">
                            ...
                            <?php endif; ?>
                            <a href="../booking/add_booking.php?room_id=<?php echo $room['id']; ?>"
...
<script>
    ...
    function fetchInteractions(roomId) {
        ...
        fetch(`../api/api_room_interactions.php?action=get_interactions&room_id=${roomId}`)
            .then(res => res.json())
            .then(data => {
                // Update Like UI
                ...
                if (data.user_logged_in) {
                    ...
                    actionArea.innerHTML = `
                        <div class="text-center bg-gray-100 p-3 rounded-lg">
                            ...
                            <a href="../auth/login.php?redirect=pages/view_rooms.php" class="text-sm font-bold text-gray-900 underline hover:text-yellow-600">Login Now</a>
                        </div>`;
                }
            });
    }
    ...
    function toggleLike() {
        ...
        fetch(`../api/api_room_interactions.php?action=toggle_like&room_id=${currentRoomId}`)
            .then(res => res.json())
            .then(data => {
                if (data.success) {
    ...
    function submitReview(e) {
        ...
        fetch('../api/api_room_interactions.php', {
            method: 'POST',
            body: formData
        })
            .then(res => res.json())
    ...
    `;
        ...
        if (room.is_booked > 0) {
            ...
            <a href="../booking/add_booking.php?room_id=${room.id}" class="block w-full py-3 bg-gray-900 text-white rounded-xl font-bold hover:bg-gray-800 transition-colors mt-4">
                Check Other Dates
            </a>
        `;
        } else {
            ...
            <a href="../booking/add_booking.php?room_id=${room.id}" class="block w-full py-3 bg-yellow-600 text-white rounded-xl font-bold hover:bg-yellow-700 transition-colors shadow-lg shadow-yellow-500/30 mt-4">
                Book This Room
            </a>
        `;
        }

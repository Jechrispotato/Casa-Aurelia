...
if ($action === 'reset') {
    ...
    $_SESSION['chat_data'] = [];
    send_response("Welcome to Casa Aurelia! How can I assist you today?", ['Book a Room', 'Room Prices', 'Hotel Info', 'View Policies']);
}
...
switch ($_SESSION['chat_state']) {
    ...
    case 'AWAIT_CHOICE':
        ...
        if (strpos($choice, 'book') !== false || strpos($choice, 'reserv') !== false) {
            ...
            send_response("<strong>Casa Aurelia</strong> represents the pinnacle of luxury living. <br><br>Located in the heart of the city with breathtaking ocean and skyline views, we offer world-class amenities including:\n<br>• High-speed fiber internet\n<br>• 24/7 Concierge & Room Service\n<br>• Premium Spa & Wellness Center\n<br>• Exclusive Rooftop Lounge", ['Book a Room', 'Room Prices', 'Back to Menu']);
    ...
    case 'AWAIT_CHECKIN_TIME':
        ...
        if ($time && $time !== '00:00:00') {
            $check_in_datetime = $_SESSION['chat_data']['check_in_date'] . ' ' . $time;
            $_SESSION['chat_data']['check_in'] = $check_in_datetime;
            $_SESSION['chat_state'] = 'AWAIT_CHECKOUT_DATE';
            send_response("Perfect! Now, what's your check-out date? (YYYY-MM-DD)", ['Cancel']);
        } else {
            send_response("Invalid time format. Please enter a valid time (e.g., 10:30 AM or 14:30).", ['Cancel']);
        }
        break;
    ...
    case 'AWAIT_CHECKOUT_DATE':
        ...
        if (preg_match('/^\d{4}-\d{2}-\d{2}$/', $input)) {
            $check_in_date = $_SESSION['chat_data']['check_in_date'];
            if (strtotime($input) < strtotime($check_in_date)) {
                send_response("Check-out date must be after check-in date ({$check_in_date}).", ['Cancel']);
            } else {
                $_SESSION['chat_data']['check_out_date'] = $input;
                $_SESSION['chat_state'] = 'AWAIT_CHECKOUT_TIME';
                $times = ['09:00 AM', '11:00 AM', '12:00 PM', '02:00 PM', '03:00 PM', '06:00 PM', 'Cancel'];
                send_response("What time will you be checking out?", $times);
            }
        } else {
            send_response("Invalid date format. Please use YYYY-MM-DD (e.g., 2025-12-26).", ['Cancel']);
        }
        break;
    ...
    case 'AWAIT_CHECKOUT_TIME':
        ...
        $time = date('H:i:s', strtotime($input));
        if ($time && $time !== '00:00:00') {
            $check_out_datetime = $_SESSION['chat_data']['check_out_date'] . ' ' . $time;
            $_SESSION['chat_data']['check_out'] = $check_out_datetime;

            // Validate check-out is after check-in
            if (strtotime($check_out_datetime) <= strtotime($_SESSION['chat_data']['check_in'])) {
                send_response("Check-out time must be after check-in time. Please select a later time.", ['Cancel']);
            } else {
                $_SESSION['chat_state'] = 'AWAIT_GUESTS';
                send_response("How many guests will be staying?", ['1', '2', '3', '4', '5+', 'Cancel']);
            }
        } else {
            send_response("Invalid time format. Please enter a valid time.", ['Cancel']);
        }
        break;
    ...
    case 'AWAIT_GUESTS':
        ...
        // Handle "5+" input
        $guests = ($input === '5+') ? 5 : (int) $input;
        if ($guests > 0 && $guests <= 10) {
            $_SESSION['chat_data']['guests'] = $guests;
            $_SESSION['chat_state'] = 'AWAIT_ROOM';

            // Fetch available rooms
            $rooms_query = mysqli_query($conn, "SELECT id, room_name FROM rooms");
            $room_options = [];
            while ($room = mysqli_fetch_assoc($rooms_query)) {
                $room_options[] = ['id' => $room['id'], 'text' => $room['room_name']];
            }
            $room_options[] = 'Cancel';

            send_response("Great! Please select a room:", $room_options);
        } else {
            send_response("Please enter a valid number of guests (1-10).", ['Cancel']);
        }
        break;
    ...
    case 'AWAIT_ROOM':
        ...
        // Search room with improved matching
        $result = mysqli_query($conn, "SELECT * FROM rooms");
        $available_rooms = [];

        while ($row = mysqli_fetch_assoc($result)) {
            $available_rooms[] = $row;
            $normalized_room_name = strtolower($row['room_name']);
            ...
            // Full or partial name match
                ...
                stripos($normalized_room_name, $normalized_input) !== false ||
                stripos($normalized_input, $normalized_room_name) !== false
            ) {
                $matches = true;
            }
            // Handle common typos/variations (e.g., "clock" for "o'clock")
            else if (
                strpos($normalized_room_name, "o'clock") !== false &&
                strpos($normalized_input, "clock") !== false
            ) {
                $matches = true;
            }
            // Match by individual words
            else {
                $room_words = explode(' ', $normalized_room_name);
                $input_words = explode(' ', $normalized_input);
                foreach ($input_words as $iword) {
                    if (strlen($iword) >= 3) { // Only match words with 3+ chars
                        foreach ($room_words as $rword) {
                            if (stripos($rword, $iword) !== false || stripos($iword, $rword) !== false) {
                                $matches = true;
                                break 2;
                            }
                        }
        ...
        if ($room_id) {
            ...
            $room_list = array_map(function ($r) {
                return ['id' => $r['id'], 'text' => $r['room_name']];
            }, $available_rooms);

            send_response("Room not found. Please select from the available rooms below:", $room_list);
        }
        break;

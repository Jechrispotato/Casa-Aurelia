document.addEventListener('DOMContentLoaded', function () {
    ...
    // Determine the site root from the script's src attribute
    const scripts = document.getElementsByTagName('script');
    let siteRoot = '';
    for (let i = 0; i < scripts.length; i++) {
        if (scripts[i].src && scripts[i].src.includes('widget_script.js')) {
            // Extract path up to and including Casa-Aurelia
            const match = scripts[i].src.match(/(.*\/Casa-Aurelia\/)/);
            if (match) {
                siteRoot = match[1];
            }
            break;
        ...
        options.forEach(opt => {
            ...
            btn.onclick = () => {
                if (text.toLowerCase() === 'view rooms') {
                    window.location.href = siteRoot + 'pages/view_rooms.php';
                } else {
    ...
    function sendMessage(message, action = '') {
        ...
        fetch(siteRoot + 'api/api_handler.php', {
            method: 'POST',
